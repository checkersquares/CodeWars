// See https://aka.ms/new-console-template for more information
using RomanNumeralConverter.RomanNumerals;

Console.WriteLine("Running Roman Numeral Converter");

//RomanNumerals.WriteNumber(2);
//RomanNumerals.WriteNumber(4);
//RomanNumerals.WriteNumber(6);
//RomanNumerals.WriteNumber(9);
//RomanNumerals.WriteNumber(26);
//RomanNumerals.WriteNumber(29);
//RomanNumerals.WriteNumber(200);
//RomanNumerals.WriteNumber(2004);
//RomanNumerals.WriteNumber(2023);
//RomanNumerals.WriteNumber(3423);
//RomanNumerals.WriteNumber(4650);

RomanNumerals.WriteDecimal("MMIX");
RomanNumerals.WriteDecimal("MDCLXVI");
RomanNumerals.WriteDecimal("CD");
RomanNumerals.WriteDecimal("XL");

Console.ReadKey();

